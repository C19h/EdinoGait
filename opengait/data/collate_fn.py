...
class CollateFn(object):
    def __init__(self, label_set, sample_config):
        ...
        self.dropout = sample_config['dropout']
    ...
    def __call__(self, batch):
        ...
        global count
        count = 0
        def sample_frames(bt):
            global count
            seqs = bt[0]
            sampled_fras = [[] for i in range(feature_num)]
            seq_len = len(seqs[0])
            trim_length = int(self.dropout * seq_len)

            indices = list(range(trim_length, seq_len - trim_length))
            seq_len = len(indices)
        ...
        # g: gpus_num
        fras_batch = [sample_frames(bt) for bt in batch]  # [b, f]
