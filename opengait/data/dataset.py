...
class DataSet(tordata.Dataset):
    ...
    def __loader__(self, paths):
        ...
        for pth in paths:
            if pth.endswith('gray.npz'):
                ...
                data_list.append(gray)
            if pth.endswith('continuous.npz'):
                output = load_npz(pth)
                continuous = output.toarray().reshape(-1, 64, 64, 8).transpose(0, 3, 1, 2)
                data_list.append(continuous)
            if pth.endswith('.pkl'):
                if pth.endswith('bboxes.pkl'):
                    with open(pth, 'rb') as f:
                        output = pickle.load(f)
                    if not isinstance(output, np.ndarray):
                        data_list.append(output['bboxes'].numpy())
                    else:
    ...
    def __dataset_parser(self, data_config, training):
        if training:
            dataset_root = data_config['dataset_root_train']
        else:
            dataset_root = data_config['dataset_root_eval']
        try:
            ...
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        ...
                        seq_dirs = sorted([f for f in os.listdir(seq_path) if f.endswith('.pkl') or f.endswith('.npz')])
                        if seq_dirs != []:
