...
class BigGait__Dinov2_Gaitbase(BaseModel):
    ...
    def forward(self, inputs):
        ...
        if ipts[0].size(-1) != 4:
            ...
            bb = ipts[0]
            ratios = bb[:, :, -2] / bb[:, :,-1]  # real_image_ratios     shape: [n,s,ratio];     ratio: w/h,  e.g. 112/224=0.5;
        del ipts
        ...
        with torch.no_grad():
            ...
            if h == 2*w:
                outss = self.preprocess(sils, self.image_size)                                           # [ns,c,448,224]    if have used pad_resize for input images
            else:
                outss = self.preprocess(padding_resize(sils, ratios, 256, 128), self.image_size)         # [ns,c,448,224]    if have not used pad_resize for input images
            # vis_gray(outss[90, 0, ...].detach().cpu().numpy(), './t.png')
            outs = self.backbone(outss, is_training=True) # [ns,h*w,c]
...
class BigGait__Dinov2_Gaitbase_Prid(BaseModel):
    def build_network(self, model_cfg):
        # get pretained models
        self.pretrained_dinov2 = model_cfg["pretrained_dinov2"]
        self.pretrained_mask_branch = model_cfg["pretrained_mask_branch"]
    ...
    def forward(self, inputs):
        if self.training:
            if self.iteration == 100 and '.pt' in self.pretrained_mask_branch:
                self.init_Mask_Branch()
...
def vis_f(f, ind, path):
    f4_numpy = f.cpu().numpy()  # 将张量转换为 numpy 数组
