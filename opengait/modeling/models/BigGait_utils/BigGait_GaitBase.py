...
class Baseline1(nn.Module):
    def __init__(self, model_cfg):
        ...
        model_cfg['backbone_cfg']['in_channel'] = model_cfg['Action_Branch']['target_dim']
        # model_cfg['backbone_cfg']['in_channel'] = 6
        self.pre_act = SetBlockWrapper(Pre_ResNet9(**model_cfg['backbone_cfg']))

        model_cfg['backbone_cfg']['in_channel'] = model_cfg['Appearance_Branch']['target_dim']
        self.pre_app = SetBlockWrapper(Pre_ResNet9(**model_cfg['backbone_cfg']))
    ...
    def vis_forward(self, appearance, action, seqL):
        action = self.pre_act(action)  # [n, c, s, h, w]
        appearance = self.pre_app(appearance)  # [n, c, s, h, w]
        outs = self.fusion([action, appearance])
        return action, appearance, outs

    def forward(self, appearance, action, seqL):
        action = self.pre_act(action)  # [n, c, s, h, w]
        appearance = self.pre_app(appearance)  # [n, c, s, h, w]
        outs = self.fusion([action, appearance])
        # heat_mapt = rearrange(outs, 'n c s h w -> n s h w c')
...
class Baseline2(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.Backbone = SetBlockWrapper(ResNet9(**model_cfg['backbone_cfg']))
        self.FCs = SeparateFCs(**model_cfg['SeparateFCs'])
        self.BNNecks = SeparateBNNecks(**model_cfg['SeparateBNNecks'])
        self.TP = PackSequenceWrapper(torch.max)
        self.HPP = HorizontalPoolingPyramid(bin_num=model_cfg['bin_num'])
