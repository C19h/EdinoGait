...
class BaseModel(MetaModel, nn.Module):
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            if not ok:
                continue
            # print(f'iteration: {model.iteration}, loss: {loss_info}')
            print(f'iteration: {model.iteration}, loss: {loss_sum}')
    ...
    def run_test(model):
        ...
        if evaluator_cfg['sampler']['batch_size'] % torch.distributed.get_world_size() != 0:
