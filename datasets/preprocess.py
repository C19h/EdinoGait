import os
from tqdm import tqdm
from utils import preprocess
from joblib import Parallel, delayed
root_dir = './db/HNU-Gait'

def main():
    tasks = []
    for c in os.listdir(root_dir):
        c_path = os.path.join(root_dir, c)
        for id_dir in os.listdir(c_path):
            id_path = os.path.join(c_path, id_dir)
            if os.path.isdir(id_path):
                for type_dir in os.listdir(id_path):
                    type_path = os.path.join(id_path, type_dir)
                    if os.path.isdir(type_path):
                        for view_dir in os.listdir(type_path):
                            view_path = os.path.join(type_path, view_dir)
                            tasks.append((view_path))

    Parallel(n_jobs=8
             )(
        delayed(preprocess)(tasks[i]) for i in tqdm(range(len(tasks))))


def check():
    for c in os.listdir(root_dir):
        c_path = os.path.join(root_dir, c)
        for id_dir in os.listdir(c_path):
            id_path = os.path.join(c_path, id_dir)
            if os.path.isdir(id_path):
                for type_dir in os.listdir(id_path):
                    type_path = os.path.join(id_path, type_dir)
                    if os.path.isdir(type_path):
                        for view_dir in os.listdir(type_path):
                            flow_f = os.path.join(type_path, view_dir, 'flow.npz')
                            gray_f = os.path.join(type_path, view_dir, 'gray.npz')
                            # xt_f = os.path.join(type_path, view_dir, 'xtore.npz')
                            delete(flow_f)
                            delete(gray_f)
                            # delete(xt_f)


def delete(f):
    if os.path.isfile(f):
        # os.remove(f)
        # print(f"removed: {f}")
        pass
    else:
        print(f"file: {f} no exist")

if __name__ == '__main__':
    main()
    # check()
    print('finish')
